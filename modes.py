...
Prices and impact numbers below (base fare, cost per mile/minute, carbon,
energy, nature-vibez, morality) are reasonable NYC approximations, not live
quotes -- Uber and Lyft don't offer open fare APIs and Curb has no public
API at all (see README).
...
``google_mode`` (driving / bicycling / transit / walking) and `Mode.estimate`
uses the real route when `route_info` is passed in, falling back to the
rate-card formula (avg speed + route factor) otherwise.
"""
...
MODES = [
    Mode(
        ...
        google_mode="driving",
        notes="Fare estimated from typical NYC UberX rates; distance and drive "
